...
\subsubsection{Eigenschaften}    
 ...
 \begin{enumerate}
	\item $\beta_1=0$\quad$\beta_3=1$\quad $h(\beta_3)=f\Bigl(\vec{x}^{(i)} - \beta_3 \nabla f(\vec{x}^{(i)})\Bigr)$
	...
	\item Parabel konstruieren: $P(\beta)=A\beta^2+B\beta+C$\quad mit \quad $\beta_1=0$,\quad $\beta_2=\beta_3/2$\\
  	...
  	\item $if\Bigl(P(\beta_3)<P(-B/2A)=\frac{\beta_3\bigl(3h(\beta_1)-4h(\beta_2)+h(\beta_3)\bigr)}{4\bigl(h(\beta_1)-h(\beta_2)h(\beta_3)\bigr)}\Bigr)\Big\{\hat{a}=\beta_3\Big\}$\\
  	$else\Big\{\hat{a}=P(-B/2A)=\frac{\beta_3\bigl(3h(\beta_1)-4h(\beta_2)+h(\beta_3)\bigr)}{4\bigl(h(\beta_1)-h(\beta_2)h(\beta_3)\bigr)}\Big\}$
